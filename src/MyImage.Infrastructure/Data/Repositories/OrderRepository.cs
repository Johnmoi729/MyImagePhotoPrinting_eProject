...
public class OrderRepository : IOrderRepository
{
    ...
    /// <summary>
    /// Get all orders regardless of status for admin management.
    /// FIXES the critical issue where admin could only see pending orders.
    /// </summary>
    public async Task<PagedResult<Order>> GetAllOrdersAsync(int page, int pageSize)
    {
        try
        {
            var skip = (page - 1) * pageSize;
            ...
            _logger.LogDebug("Retrieved {Count} orders (page {Page} of {PageSize})",
                orders.Count, page, pageSize);
        ...
        {
            _logger.LogError(ex, "Failed to get all orders for admin");
            throw;
        }
    }
    ...
    /// Critical for webhook processing and payment status updates.
    /// </summary>
    public async Task<Order?> GetByPaymentIntentIdAsync(string paymentIntentId)
    {
        try
        {
            var filter = Builders<Order>.Filter.Eq("payment.stripePaymentIntentId", paymentIntentId);
            var order = await _orders.Find(filter).FirstOrDefaultAsync();

            if (order != null)
            {
                _logger.LogDebug("Found order {OrderNumber} for Payment Intent {PaymentIntentId}",
                    order.OrderNumber, paymentIntentId);
            }
            else
            {
                _logger.LogDebug("No order found for Payment Intent {PaymentIntentId}", paymentIntentId);
            }
        ...
        {
            _logger.LogError(ex, "Error finding order by Payment Intent ID: {PaymentIntentId}", paymentIntentId);
            throw;
        }
    }

    /// <summary>
    /// Get orders created within a specific date range.
    /// Used for admin reporting and analytics.
    /// </summary>
    public async Task<PagedResult<Order>> GetOrdersByDateRangeAsync(DateTime fromDate, DateTime toDate, int page, int pageSize)
    {
        try
        {
            var filter = Builders<Order>.Filter.And(
                Builders<Order>.Filter.Gte("metadata.createdAt", fromDate),
                Builders<Order>.Filter.Lte("metadata.createdAt", toDate)
            );
            ...
            _logger.LogDebug("Retrieved {Count} orders from {FromDate} to {ToDate}",
        ...
        {
            _logger.LogError(ex, "Failed to get orders by date range");
            throw;
        }
    }
